...
void main(void)
{
    ...
    vec2 coord = ex_TexCoord+(uvOffset);
        
    coord.y += 0.02*sin(time)*(sin(coord.y));
    coord.x += 0.02*cos(time)*(cos(coord.y));
    
    vec4 texel = texture(DiffuseMap, coord );
    vec4 normalTexel = texture(NormalMap, coord);
    ...
    out_Color = vec4((color.rgb * texel.rgb), (material_diffuse.a+texel.a)/2.0); //texture and lighting
    
    //if (dot(out_Color, vec4(1)) > 1.5){
    //    out_Color = vec4(out_Color.rgb*1.2, out_Color.a);
    //}else{
        out_Color = vec4(out_Color.rgb/3.0, out_Color.a);
    //}
